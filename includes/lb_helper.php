...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '3849F1A7';
		$this->api_url = 'https://project.sharpdev.in/dtssgguyrdfjk/';
		$this->api_key = 'F8B072301DE063703DE9';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["7e9f8b5ceb3dc80"])){
				$_SESSION["7e9f8b5ceb3dc80"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["7e9f8b5ceb3dc80"])){
				$get_data = $this->call_api(
					'POST',
